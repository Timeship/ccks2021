...
class InputExample(object):
    ...
    def __init__(self, example_id, tokens, triggerL, triggerR, title,label=None):
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        textL = ['[unused0]']+tokenizer.tokenize(" ".join(example.tokens[:example.triggerL]))+tokenizer.tokenize(" ".join(example.tokens[example.triggerL:example.triggerR]))
        ...
        textHead =['[unused3]']+tokenizer.tokenize(" ".join(example.title))
        maskL = [1.0 for i in range(0,len(textL)+1)] + [0.0 for i in range(0,len(textR+textHead)+2)]
        maskR = [0.0 for i in range(0,len(textL)+1)] + [1.0 for i in range(0,len(textR+textHead)+2)]
        ...
        inputs = tokenizer.encode_plus(
            textL + ['[unused2]'] + textR+textHead, add_special_tokens=True, max_length=max_length, return_token_type_ids=True
        )
